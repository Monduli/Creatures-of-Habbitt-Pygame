...
class Crawler():
    ...
    def start(self):
        ...
        self.player = PlayerMap(character, self.screen, width/2/2, height/2/2, get_portrait("N. Steen"))
    ...
    def input(self, key = None, pressed = False):
        if pressed != False:
            ...
            if (keys[pygame.K_RIGHT] or keys[pygame.K_d]) and self.player.x < 1015:
                if self.speed_x < 10:
                    self.speed_x += 1
                self.player.x += self.speed_x
            if (keys[pygame.K_DOWN] or keys[pygame.K_s]) and self.player.y > 133:
                if self.speed_y > -10:
                    self.speed_y -= 1
                self.player.y += self.speed_y
        else:
    ...
    def draw_gl_scene(self, dungeon_rooms, current_room):
        ...
        blit_bg(0, dungeon_rooms[current_room], False)
        self.player.draw()
        self.enemy.draw()
