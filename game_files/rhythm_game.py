...
class Board(object):
    ...
    def __init__(self, width, height, background):
        ...
        for shape in shapes.split():
            self.shapes.append(loadify('images/tabs/{}_gem_new.png'.format(shape)))
...
class MatchGame(object):
    ...
    def process_action(self, item, party, enemy, player_active, enemy_active, turns, enemy_turns):
        """
        The function `process_action` takes in various parameters and performs different actions based
        on the value of the `item` parameter.
        
        :param item: The "item" parameter represents the action or item that the player wants to use. It
        could be a string indicating the type of action, such as "red" for a physical attack, "purple"
        for a magic attack, "green" for healing, "orange" for debuffing the
        :param party: The "party" parameter represents the list of active party members
        :param enemy: The "enemy" parameter is a list that contains all the enemy characters in the game
        :param player_active: The parameter "player_active" refers to the active player character in the
        game
        :param enemy_active: enemy_active is the currently active enemy in the battle
        :param turns: The "turns" parameter represents the list of turns that have occurred in the
        battle so far. Each turn is typically represented as a dictionary or object that contains
        information about the action taken by a character or enemy
        :param enemy_turns: The parameter "enemy_turns" is a list that keeps track of the turns of the
        enemy characters. It is used to determine the order in which the enemy characters take their
        actions
        :return: the variable "update_text".
        """
    ...
    def draw_gl_scene(self, party_current, xp=None, update_text=None):
        ...
        if len(party) > 0:    
            #if self.talking == 1:
                #self.blit_image([WINDOW_WIDTH, WINDOW_HEIGHT], width-580,height-90, self.current_talking_portrait(), 1, 1, 1)
            #else:
            self.blit_image([WINDOW_WIDTH, WINDOW_HEIGHT], width-98,height-95, party[0].get_portrait().convert_alpha(), 1, 1, 1)
        if len(party) > 1:    
            #if self.talking == 2:
            #    self.blit_image([WINDOW_WIDTH, WINDOW_HEIGHT], width-580,height-90, self.current_talking_portrait("nsteen"), 1, 1, 1)
            #else:
            self.blit_image([WINDOW_WIDTH, WINDOW_HEIGHT], width-98,height-210, party[1].get_portrait().convert_alpha(), 1, 1, 1)
        if len(party) > 2:
            self.blit_image([WINDOW_WIDTH, WINDOW_HEIGHT], width-98,height-325, party[2].get_portrait().convert_alpha(), 1, 1, 1)
        if len(party) > 3:
            self.blit_image([WINDOW_WIDTH, WINDOW_HEIGHT], width-98,height-440, party[3].get_portrait().convert_alpha(), 1, 1, 1)

        if len(enemy) > 0:
            self.blit_image([WINDOW_WIDTH, WINDOW_HEIGHT], width-98,height-555, get_portrait(enemy[0].get_name()).convert_alpha(), 1, 1, 1)
        if len(enemy) > 1:
            self.blit_image([WINDOW_WIDTH, WINDOW_HEIGHT], width-98,height-670, get_portrait(enemy[1].get_name()).convert_alpha(), 1, 1, 1)
        if len(enemy) > 2:
            self.blit_image([WINDOW_WIDTH, WINDOW_HEIGHT], width-98,height-785, get_portrait(enemy[2].get_name()).convert_alpha(), 1, 1, 1)   

        #self.update_box()     

        if self.end_fade == 1:
            self.blit_image([width, height], 0-self.counter_x, 0, pygame.image.load("images/black_pass.png").convert_alpha(), 1, 1, 1)
            self.counter_x += 100
            if self.counter_x >= 1700:
                self.end_fade = 0

        if self.start_fade == 1:
            self.blit_image([width, height], width-self.counter_x, 0, pygame.image.load("images/black_pass.png").convert_alpha(), 1, 1, 1)
    ...
    def crawler_fade_in(self):
        ...
        self.blit_image((1600,900), 7200-self.counter_x/2, 4050-self.counter_y/2, fade, 1,1,1)
        pygame.display.flip()
    ...
    def scoot(self, counter_x):
        ...
        while transfer == 1:
            self.blit_image((1600,900), width-counter_x, height, black_pass, 1,1,1)
            counter_x += 1
            if counter_x >= 1600:
                transfer = 0
    ...
    def blit_image(self, size, x, y, source, r, g, b):
        self.display.blit(source, (x, y))
